...
//================================================================================
/*
void svd(SpMatrix& A, Matrix& U, SpIntMatrix& D, Matrix& V, Mesh& m, Vector& w, double tol, int maxit)
{
  if(myRank == 0)
    printf("== SVD Golub\n");

  srows = A.rows(); scols = A.cols();
  Bidiagonale(A,U,D,V);
  ...
  for (int i = 0; i < scols; ++i)
    for (int j = 0; j < scols; ++j)
      V(i,j) = (i==j) ? 1:0;
  double err1 = 10, err2 = 10;
  int iter = 0;
  while (err2 >= 1.0e-6 && iter <= 1.0e8)
  {
    Vector u(D.cols()), v(D.rows());
    ...
    err1 = norm(tril1(D,-1), 'fro');
    err2 = norm(triu1(D,1), 'fro') + err1;
    iter++;
  }
}
*/
