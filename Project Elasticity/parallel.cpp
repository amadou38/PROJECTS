...
{
  MPI_Request *requestSnd1, *requestSnd2;
  MPI_Request *requestRcv1, *requestRcv2;
  MPI_Status status;
  requestSnd1 = new MPI_Request[nbTasks];
  requestRcv1 = new MPI_Request[nbTasks];
  requestSnd2 = new MPI_Request[nbTasks];
  requestRcv2 = new MPI_Request[nbTasks];

  double **bufferSnd1, **bufferSnd2;
  double **bufferRcv1, **bufferRcv2;
  bufferSnd1 = new double*[nbTasks];
  bufferRcv1 = new double*[nbTasks];
  bufferSnd2 = new double*[nbTasks];
  bufferRcv2 = new double*[nbTasks];

  
  for(int nTask=0; nTask<nbTasks; nTask++){
    int numToExch = m.numNodesToExch(nTask);
    if(numToExch > 0){
      bufferSnd1[nTask] = new double[numToExch];
      bufferRcv1[nTask] = new double[numToExch];
      bufferSnd2[nTask] = new double[numToExch];
      bufferRcv2[nTask] = new double[numToExch];
      for(int nExch=0; nExch<numToExch; nExch++){
        bufferSnd1[nTask][nExch] = vec(m.nodesToExch(nTask,nExch)-1);
        bufferSnd2[nTask][nExch] = vec(m.nbOfNodes+m.nodesToExch(nTask,nExch)-1);
      }
      MPI_Isend(bufferSnd1[nTask], numToExch, MPI_DOUBLE, nTask, 0, MPI_COMM_WORLD, &requestSnd1[nTask]);
      MPI_Irecv(bufferRcv1[nTask], numToExch, MPI_DOUBLE, nTask, 0, MPI_COMM_WORLD, &requestRcv1[nTask]);
      MPI_Isend(bufferSnd2[nTask], numToExch, MPI_DOUBLE, nTask, 0, MPI_COMM_WORLD, &requestSnd2[nTask]);
      MPI_Irecv(bufferRcv2[nTask], numToExch, MPI_DOUBLE, nTask, 0, MPI_COMM_WORLD, &requestRcv2[nTask]);
    }
  }
  ...
  for(int nTask=0; nTask<nbTasks; nTask++){
    int numToExch = m.numNodesToExch(nTask);
    if(numToExch > 0){
      MPI_Wait(&requestRcv1[nTask], &status);
      MPI_Wait(&requestRcv2[nTask], &status);
      for(int nExch=0; nExch<numToExch; nExch++){
        vec(m.nodesToExch(nTask,nExch)-1) += bufferRcv1[nTask][nExch];
        vec(m.nbOfNodes+m.nodesToExch(nTask,nExch)-1) += bufferRcv2[nTask][nExch];
      }
      delete bufferRcv1[nTask], bufferRcv2[nTask];
      MPI_Wait(&requestSnd1[nTask], &status);
      MPI_Wait(&requestSnd2[nTask], &status);
      delete bufferSnd1[nTask];
      delete bufferSnd2[nTask];
    }
  }
