...
double F(double(*f)(IntVector Ind, double x, double y, Vector Xe, double he), double x, double y, IntVector indX, IntVector indY, Vector Xe, double he)
{
...
double integral_fct(double(*f)(IntVector Ind, double x, double y, Vector Xe, double he),  IntVector Ind, Matrix Q, Vector Xe, double he, int prec)
{
	double Integ = 0;
	Matrix T(2,3);
	...
	for (int i = 0; i < Q.rows()-1; i++)
	{
		...
		Quad2D(prec, T, W, X);
		Vector Z(2);
        IntVector indX(2), indY(2); indX(1) = Ind(0); indY(1) = Ind(1);
        for (int l = 0; l < 2; l++)
        {
            indX(0) = l; indY(0) = l;
            Z(l) = F(f, X(0,0), X(0,1), indX, indY, Xe, he);
        }
        FF = Z(0) + Z(1);
		Integ += sum(W*FF);
	}
